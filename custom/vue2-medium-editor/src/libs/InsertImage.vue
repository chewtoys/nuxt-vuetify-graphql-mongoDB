...
export default {
  ...
  props: ['editor', 'uploadUrl', 'editorRef', 'onChange'],
  methods: {
    subscribe() {
    ...
    detectShowToggle(e) {
      ...
      if (e.target.className.indexOf('editor-image-description') <= -1) {
        ...
        for (let i = 0; i < editorImages.length; i++) {
          editorImages[i].className = editorImages[i].className.replace(
            'is-focus',
            ''
          )
        }
    ...
    addImage(url) {
      ...
      if (this.insert.isToggle) {
        ...
        this.handler.currentImg = this.editor
          .getSelectedParentElement()
          .previousElementSibling.previousElementSibling.querySelector('img')
    ...
    detectEmbed(e) {
      if (e.keyCode === 13 && this.insert.embedElm) {
        this.insert.embedElm.innerHTML = this.insert.embedElm.innerHTML.replace(
          '<br>',
          ''
        )
